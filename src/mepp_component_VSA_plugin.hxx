  ...
  public mepp_component_plugin_interface
{
	...
	public:
		...
		void init(mainwindow* mainWindow, QList<QMdiSubWindow *> lw)
		{
			...
			// début --- actions ---
			actionSegmentation = new QAction(tr("Burst Wind Segmentation"), this);
			if (actionSegmentation)
				connect(actionSegmentation, SIGNAL(triggered()), this, SLOT(BurstWindSegmentation2()));
	...
	public slots:
		...
		/**
		 \fn	void mepp_component_VSA_plugin::BurstWindSegmentation();

		 \brief	Launch the Variational segmentation.


		 */
		void BurstWindSegmentation();
		void BurstWindSegmentation2();
		...
		void write_LabelReg_in_Text(string output_name, PolyhedronPtr pMesh);
		void write_DepthReg_in_Text(string output_name, PolyhedronPtr pMesh);
